...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_badge_quests_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_user_quest_progress_updated_at'
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE policyname = 'Users can view all badge quests' AND tablename = 'badge_quests'
  ) THEN
    DROP POLICY "Users can view all badge quests" ON badge_quests;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can create badge quests' AND tablename = 'badge_quests'
  ) THEN
    DROP POLICY "Users can create badge quests" ON badge_quests;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can update their own badge quests' AND tablename = 'badge_quests'
  ) THEN
    DROP POLICY "Users can update their own badge quests" ON badge_quests;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can delete their own badge quests' AND tablename = 'badge_quests'
  ) THEN
    DROP POLICY "Users can delete their own badge quests" ON badge_quests;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can view all badge quest tasks' AND tablename = 'badge_quest_tasks'
  ) THEN
    DROP POLICY "Users can view all badge quest tasks" ON badge_quest_tasks;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can insert badge quest tasks' AND tablename = 'badge_quest_tasks'
  ) THEN
    DROP POLICY "Users can insert badge quest tasks" ON badge_quest_tasks;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can delete badge quest tasks' AND tablename = 'badge_quest_tasks'
  ) THEN
    DROP POLICY "Users can delete badge quest tasks" ON badge_quest_tasks;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can view their own quest progress' AND tablename = 'user_quest_progress'
  ) THEN
    DROP POLICY "Users can view their own quest progress" ON user_quest_progress;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can insert their own quest progress' AND tablename = 'user_quest_progress'
  ) THEN
    DROP POLICY "Users can insert their own quest progress" ON user_quest_progress;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can update their own quest progress' AND tablename = 'user_quest_progress'
  ) THEN
    DROP POLICY "Users can update their own quest progress" ON user_quest_progress;
  END IF;
END $$;
